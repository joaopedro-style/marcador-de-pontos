  ...
  const renderItem = ({ item }: { item: PontoSalvo }) => (
    <View style={estilos.itemHistorico}>
      <Text style={estilos.itemTexto}>
        **Data/Hora:** {new Date(item.dataHora).toLocaleDateString()} -{" "}
        {new Date(item.dataHora).toLocaleTimeString()}
      </Text>
      <Text style={estilos.itemTexto}>**Endereço:** {item.rua}</Text>
      <Text style={estilos.itemTexto}>
        **Lat/Lon:** {item.latitude.toFixed(6)}, {item.longitude.toFixed(6)}
      </Text>
    </View>
  );
  ...
  return (
    <>
      <ScrollView contentContainerStyle={estilos.container}>
        ...
        <Pressable style={estilos.botaoHistorico} onPress={alternarModal}>
        ...
        <View style={estilos.modalContainer}>
          <View style={estilos.modalContent}>
            ...
            {pontosSalvos.length > 0 ? (
              ...
              <Text style={estilos.historicoVazio}>
                Nenhum ponto registrado ainda.
              </Text>
            )}
            <Pressable style={estilos.botaoFechar} onPress={alternarModal}>
